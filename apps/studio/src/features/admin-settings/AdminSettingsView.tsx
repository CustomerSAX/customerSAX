...
type AdminProject = { id: string; projectKey: string; displayName?: string | null; smtpProfileId?: string | null };
...
const WORKSPACE_ADMIN = gql`
  query WorkspaceAdmin($clientId: ID!, $projectKey: String!) {
    ...
    adminProjectsByClient(clientId: $clientId) { id projectKey displayName smtpProfileId }
...
function Users({ data, clientId, projectKey, actor, refetch }: { data: WorkspaceAdminData; clientId: string; projectKey: string; actor: string; refetch: () => Promise<unknown> }) {
  const [search, setSearch] = useState(""); const [updateUser] = useMutation(UPDATE_USER); const roles: Role[] = data.adminRoles;
  ...
  return <section className="space-y-4"><div><h2 className="text-xl font-bold">User Management</h2><p className="text-sm text-m-text-muted">Manage roles for <strong>{projectKey}</strong>.</p></div><input aria-label="Search users" value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search by email or name…" className="w-full rounded-lg border border-m-border bg-m-surface px-3 py-2"/><div className="overflow-auto rounded-xl border border-m-border bg-m-surface"><table className="w-full text-sm"><thead><tr className="border-b border-m-border text-left"><th className="p-3">Email</th><th>Name</th><th>Role ({projectKey})</th><th>Change Role</th></tr></thead><tbody>{users.map((u)=>{const membership=u.clientProjects.find((p)=>p.projectKey===projectKey); if(!membership)return null; return <tr key={u.id} className="border-b border-m-border"><td className="p-3 font-medium">{u.email}</td><td>{[u.firstName,u.lastName].filter(Boolean).join(" ")||"--"}</td><td>{membership.role}</td><td><select aria-label={`Change role for ${u.email}`} value={membership.role} onChange={(e)=>void changeRole(u,e.target.value)} className="rounded border border-m-border bg-m-surface px-2 py-1">{roles.map(r=><option key={r.key} value={r.key}>{r.label}</option>)}</select></td></tr>})}</tbody></table></div></section>;
}

function Roles({ roles, clientId, projectKey, refetch }: { roles: Role[]; clientId: string; projectKey: string; refetch: () => Promise<unknown> }) {
  const [selectedId,setSelectedId]=useState(roles[0]?.id??""); const selected=roles.find(r=>r.id===selectedId); const [createRole]=useMutation(CREATE_ROLE); const [updateRole]=useMutation(UPDATE_ROLE); const [deleteRole]=useMutation(DELETE_ROLE);
  async function add(){const key=`custom_${Date.now()}`; await createRole({variables:{clientId,projectKey,input:{key,label:"Custom Role",description:"Custom workspace role",permissions:blankPermissions()}}}); await refetch();}
  async function toggle(module:string,action:keyof Omit<Permission,"module">){if(!selected)return; const permissions=selected.permissions.map(p=>p.module===module?{...p,[action]:!p[action]}:p); await updateRole({variables:{id:selected.id,clientId,projectKey,input:{permissions}}}); await refetch();}
  return <section className="space-y-4"><div className="flex justify-between"><div><h2 className="text-xl font-bold">Role Management</h2><p className="text-sm text-m-text-muted">Page and action permissions for {projectKey}.</p></div><button onClick={()=>void add()} className="rounded-lg bg-m-primary px-4 py-2 text-sm font-bold text-white">+ Add Role</button></div><div className="grid gap-4 lg:grid-cols-[280px_1fr]"><div className="space-y-2">{roles.map(r=><button key={r.id} onClick={()=>setSelectedId(r.id)} className={`w-full rounded-xl border p-4 text-left ${selectedId===r.id?"border-m-primary bg-m-primary-50":"border-m-border bg-m-surface"}`}><strong>{r.label}</strong><span className="ml-2 text-xs text-m-text-muted">{r.system?"System":r.key}</span><p className="mt-1 text-xs text-m-text-muted">{r.description}</p></button>)}</div>{selected&&<div className="overflow-auto rounded-xl border border-m-border bg-m-surface p-4"><div className="mb-3 flex justify-between"><h3 className="font-bold">{selected.label} ACL</h3>{!selected.system&&<button onClick={async()=>{await deleteRole({variables:{id:selected.id,clientId,projectKey}});setSelectedId("");await refetch();}} className="text-sm font-semibold text-m-error">Delete</button>}</div><table className="w-full text-sm"><thead><tr><th className="text-left">Module</th>{["view","create","update","delete"].map(a=><th key={a} className="p-2 capitalize">{a}</th>)}</tr></thead><tbody>{selected.permissions.map(p=><tr key={p.module} className="border-t border-m-border"><td className="py-2 capitalize">{p.module}</td>{(["view","create","update","delete"] as const).map(a=><td key={a} className="text-center"><input aria-label={`${selected.label} ${p.module} ${a}`} type="checkbox" checked={p[a]} onChange={()=>void toggle(p.module,a)}/></td>)}</tr>)}</tbody></table></div>}</div></section>;
}
