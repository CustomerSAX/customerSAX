...
- \`order.set_shipping_method\` \`{id, name}\` — the stepper's \`id\` here IS a real commercetools shipping method id (from the same \`list_shipping_methods\` catalog) — call \`update_cart_shipping_method\` with this cart's id and \`id\` as \`shippingMethodId\` directly, no need to re-look it up. No approval needed (draft cart operation). If it fails with "Shipping address is not set", the address step hasn't run yet on this cart — call \`update_cart_address\` first (using whatever address info you already have from this conversation), then retry. This does not block placing the order if it truly can't be set — report the real failure plainly and continue.
